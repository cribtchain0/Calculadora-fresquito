-- ════════════════════════════════════════════════════════════
-- Fresquito — fruta congelada (aprovechar sobrantes de producción),
-- Puntos de venta como reventa a consignación (contacto, teléfono,
-- ubicación con link, fecha de alta) y eliminación de la sección
-- genérica "Proveedores" (nunca se usó: 0 registros).
--
-- Se puede pegar tal cual en el SQL Editor del panel de Supabase,
-- después de 20260721120000. Es idempotente.
-- ════════════════════════════════════════════════════════════

-- ── Columnas nuevas ──────────────────────────────────────────
-- Liga una fruta fresca con su hermana "Fruta congelada" (mismo
-- insumo, ya congelado, para no perder el sobrante de una corrida).
alter table insumos add column if not exists congelado_id text;

-- Puntos de venta: quiénes reciben paletas a consignación.
alter table puntos_venta add column if not exists contacto text not null default '';
alter table puntos_venta add column if not exists telefono text not null default '';
alter table puntos_venta add column if not exists ubicacion_link text not null default '';
alter table puntos_venta add column if not exists fecha_alta date not null default current_date;
  ...
  select jsonb_build_object(
    ...
    'puntosVenta', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'ubicacionLink', ubicacion_link, 'fechaAlta', to_char(fecha_alta, 'YYYY-MM-DD')
...
begin
  ...
  insert into insumos (id, nombre, tipo, unidad, merma, stock, stock_min, costo_prom,
                       ...
                       por_paquete, unidades_por_paquete, nombre_paquete, mermas,
                       congelado_id, posicion)
  select e.v->>'id', coalesce(e.v->>'nombre', ''), coalesce(e.v->>'tipo', 'Otro'),
         ...
         coalesce(e.v->>'nombrePaquete', ''), coalesce(e.v->'mermas', '[]'::jsonb),
         nullif(e.v->>'congeladoId', ''), e.ord
  ...
  on conflict (id) do update set
    ...
    nombre_paquete = excluded.nombre_paquete, mermas = excluded.mermas,
    congelado_id = excluded.congelado_id, posicion = excluded.posicion;
  ...
  delete from puntos_venta t where not exists (
    ...
    where e.v->>'id' = t.id);
  insert into puntos_venta (id, nombre, ubicacion, adeudo, notas, eventos, inventario,
                            contacto, telefono, ubicacion_link, fecha_alta, posicion)
  select e.v->>'id', coalesce(e.v->>'nombre', ''), coalesce(e.v->>'ubicacion', ''),
         fq_num(e.v->>'adeudo'), coalesce(e.v->>'notas', ''),
         coalesce(e.v->'eventos', '[]'::jsonb), coalesce(e.v->'inventario', '{}'::jsonb),
         coalesce(e.v->>'contacto', ''), coalesce(e.v->>'telefono', ''),
         coalesce(e.v->>'ubicacionLink', ''), fq_fecha(e.v->>'fechaAlta'), e.ord
  ...
  on conflict (id) do update set
    ...
    ubicacion_link = excluded.ubicacion_link, fecha_alta = excluded.fecha_alta,
    posicion = excluded.posicion;
...
-- ── Eliminar "Proveedores" (0 registros, nunca se usó como tal:
-- lo que sí se usa es "puntos_venta", que se queda) ──────────
